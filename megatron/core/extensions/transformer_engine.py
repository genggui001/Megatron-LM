...
if HAVE_TE and is_te_min_version("1.9.0.dev0"):
    ...
    def _round_to_fp8_e4m3(x: torch.Tensor) -> torch.Tensor:
        sign = torch.sign(x)
        x_abs = torch.clamp(x.abs(), max=FP8_E4M3_MAX)

        min_normal = torch.tensor(
            FP8_E4M3_MIN_NORMAL,
            dtype=x.dtype,
            device=x.device,
        )
        min_subnormal = torch.tensor(
        ...
        is_zero = x_abs == 0
        is_subnormal = (x_abs < min_normal) & (~is_zero)

        def _round_to_even_nonnegative(y: torch.Tensor) -> torch.Tensor:
            lower = torch.floor(y)
            frac = y - lower
            should_round_up = frac > 0.5
            is_half = frac == 0.5
            lower_is_odd = (lower.to(torch.int32) & 1) != 0
            return torch.where(
                should_round_up | (is_half & lower_is_odd),
                lower + 1.0,
                lower,
            )

        subnormal = _round_to_even_nonnegative(x_abs / min_subnormal)
        subnormal = torch.clamp(subnormal, 0, 7) * min_subnormal

        normal_input = torch.where(is_subnormal | is_zero, min_normal, x_abs)
        exponent = torch.floor(torch.log2(normal_input))
        base = torch.pow(two, exponent)
        significand = normal_input / base
        mantissa = _round_to_even_nonnegative((significand - 1.0) * 8.0)
        carry = mantissa == 8
        ...
        base = torch.pow(two, exponent)
        normal = (1.0 + mantissa / 8.0) * base
        normal = torch.clamp(normal, max=FP8_E4M3_MAX)

        rounded = torch.where(is_subnormal, subnormal, normal)
        rounded = torch.where(is_zero, torch.zeros_like(rounded), rounded)
        rounded = rounded * sign
        return torch.where(
            rounded == 0,
            torch.full_like(rounded, FP8_E4M3_EPS),
            rounded,
        )

    @torch.compile
    def nvfp4_weight_qdq(
        weight: torch.Tensor,
        group_size: int = 16,
    ) -> torch.Tensor:
        """
        Simulate NVFP4-style weight quantize + dequantize (QDQ).

        Notes:
        - This is a software simulation for analysis/testing.
            - Real NVFP4 kernel/packing behavior is implemented by runtime libraries.
            - This follows the repository's FP4 E2M1 path rather than int4 QDQ.
            - FP4 values are clamped to [-6, 6] and mapped to the E2M1 codebook.
            - global_scale is stored in fp32.
            - The reference always uses two-level scaling:
                1) global tensor scale
                2) per-group weight scale stored in FP8 E4M3
        """
        if weight.ndim != 2:
        ...
        if n % group_size != 0:
            raise ValueError(
                f"Input dimension n={n} must be divisible by group_size={group_size}"
            )

        compute_weight = weight
        num_groups = n // group_size

        observed_global = compute_weight.reshape(1, 1, -1)
        global_min_vals = torch.amin(observed_global, dim=(0, -1))
        global_max_vals = torch.amax(observed_global, dim=(0, -1))
        global_min_vals = torch.min(global_min_vals, torch.zeros_like(global_min_vals))
        global_max_vals = torch.max(global_max_vals, torch.zeros_like(global_max_vals))

        global_amax = torch.max(
            torch.abs(global_min_vals),
            torch.abs(global_max_vals),
        )
        global_amax = torch.clamp(
            global_amax,
            min=torch.finfo(global_amax.dtype).tiny,
        )
        fp8_max = torch.tensor(
            FP8_E4M3_MAX,
            device=compute_weight.device,
            dtype=torch.float32,
        )
        global_scale = fp8_max * FP4_E2M1_MAX / global_amax
        ...
        observed_local = compute_weight.unflatten(-1, (num_groups, group_size)).unsqueeze(0)
        min_vals = torch.amin(observed_local, dim=(0, -1))
        max_vals = torch.amax(observed_local, dim=(0, -1))
        min_vals = torch.min(min_vals, torch.zeros_like(min_vals))
        max_vals = torch.max(max_vals, torch.zeros_like(max_vals))

        local_amax = torch.max(
            torch.abs(min_vals),
            torch.abs(max_vals),
        )
        local_scale = local_amax / FP4_E2M1_SCALE_DENOM

        weight_scale = _round_to_fp8_e4m3(global_scale * local_scale)
        weight_scale = torch.where(
            weight_scale == 0,
            torch.tensor(FP8_E4M3_EPS, dtype=weight_scale.dtype, device=weight_scale.device),
            weight_scale,
        )

        effective_scale = weight_scale / global_scale
        effective_scale = torch.where(
            effective_scale == 0,
            torch.full_like(effective_scale, torch.finfo(effective_scale.dtype).eps),
            effective_scale,
        )

        w_group = compute_weight.unflatten(-1, (num_groups, group_size))
        q_input = torch.clamp(
            w_group / effective_scale.unsqueeze(-1),
            FP4_E2M1_MIN,
            FP4_E2M1_MAX,
        )
        q_value = _cast_to_fp4_e2m1(q_input)

        dequant_group = q_value * effective_scale.unsqueeze(-1)
        dequant = dequant_group.view_as(weight).to(weight.dtype)

        return dequant


    class _FakeNVFP4QuantizationSTE(torch.autograd.Function):

        @staticmethod
        def forward(ctx, x, group_size):
            return nvfp4_weight_qdq(x, group_size)
    ...
    def fake_nvfp4_quantization_ste(x, group_size):
        x_out = _FakeNVFP4QuantizationSTE.apply(x, group_size)
    ...
    _FAKE_GROUPED_LINEAR_TYPE_ALIASES = {
        "int4": "fake_int4",
        "fake_int4": "fake_int4",
        "nvfp4": "fake_nvfp4",
        "fake_nvfp4": "fake_nvfp4",
    }


    def _normalize_fake_grouped_linear_type(fake_type: Optional[str]) -> Optional[str]:
        if fake_type is None:
            return None

        normalized_fake_type = _FAKE_GROUPED_LINEAR_TYPE_ALIASES.get(fake_type.lower())
        if normalized_fake_type is None:
            raise ValueError(
                f"Unsupported fake_type '{fake_type}'. "
                "Expected one of: fake_int4, int4, fake_nvfp4, nvfp4."
            )
        return normalized_fake_type
    ...
    class TEGroupedLinear(te.pytorch.GroupedLinear):
        ...
        def __init__(
            ...
            *,
            ...
            resolved_fake_type = _normalize_fake_grouped_linear_type(fake_type)
            resolved_fake_group_size = fake_group_size

            if resolved_fake_type is None and resolved_fake_group_size > 0:
                raise ValueError("fake_group_size requires fake_type to be set.")
            if resolved_fake_type is not None and resolved_fake_group_size <= 0:
                raise ValueError(
                    "fake_group_size must be > 0 when fake quantization is enabled."
                )

            self.fake_type = resolved_fake_type
            self.fake_quantization_type = resolved_fake_type
            self.fake_group_size = resolved_fake_group_size
            self.fake_quantization_group_size = resolved_fake_group_size
        ...
        def _get_weight_tensors(self):
            ...
            if self.fake_quantization_type == "fake_int4":
                weight_tensors = [
                    fake_int4_quantization_ste(w, self.fake_quantization_group_size)
                    for w in weight_tensors
                ]
            elif self.fake_quantization_type == "fake_nvfp4":
                weight_tensors = [
                    fake_nvfp4_quantization_ste(w, self.fake_quantization_group_size)
                    for w in weight_tensors
                ]
        ...
        def __init__(
            ...
            *,
            ...
            super().__init__(
                ...
                fake_type=fake_type,
                fake_group_size=fake_group_size,
            )
